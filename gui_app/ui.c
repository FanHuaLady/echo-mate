...
ui_app_data_t ui_apps[_APP_NUMS] = 
{
    {
        .name = "HomePage",                                         // 界面名称
        .init = ui_HomePage_init,                                   // 界面初始化函数
        .deinit = ui_HomePage_deinit,                               // 界面销毁函数
        .page_obj = NULL
    },
    {
        .name = "SettingPage",
...
void ui_init(void)
{
    ...
    lv_lib_pm_Init(&page_manager);                                  // 初始化页面管理器
    lv_lib_pm_page_t *pm_page[_APP_NUMS];                           // 页面指针数组
    for(int i = 0; i < _APP_NUMS; i++)                              // 创建所有页面
    {
        pm_page[i] = lv_lib_pm_CreatePage(&page_manager, ui_apps[i].name, ui_apps[i].init, ui_apps[i].deinit, NULL);
    }
    lv_lib_pm_OpenPage(&page_manager, NULL, "HomePage");            // 打开首页
